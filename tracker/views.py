...
def home(request):
    ...
    members = Member.objects.filter(Q(role='Member') | Q(role='Co-ordinator'))
...
def add_members(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            qs=Member.objects.filter(name=name)
            if len(qs)==0:
        ...
        return render(request,'add_member.html',{'form':MemberForm(),'msg':msg})
    return render(request,'add_member.html',{'form':form,'msg':msg})

def view_members(request):
    qs=Member.objects.all()
    msg=''
    if len(qs)!=0:
        return render(request,'view_members.html',{'qs':qs,'msg':msg})
    else:
        return render(request,'view_members.html',{'msg':"No member registered yet"})

def view_meetings(request):
    qs=Meeting.objects.all()
    msg=''
    if len(qs)!=0:
        return render(request,'view_meetings.html',{'qs':qs,'msg':msg})
    else:
        return render(request,'view_meetings.html',{'msg':"No meetings added yet !"})
...
def upload_attendance_file(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            for i in attendees:
                if len(i)>0:
                    ...
                    if not members:
                        ...
                        continue
                    x=Attendance.objects.filter(member_name=member_name,first_seen=first_seen,duration=duration,meeting_code=meeting_code).first()
                    if x:
            ...
            qs=Meeting.objects.filter(code=meeting_code)
            if len(qs)==0:
            ...
            return render(request,'upload_attendance_file.html',{'form':AttendanceFileForm(),'msg':msg})
        else:
            msg='Error in Form Validation'
    return render(request,'upload_attendance_file.html',{'form':form,'msg':msg}) 
   
def add_minutes(request,code):
    form=AddMinutes()
    msg=''
    qs=Meeting.objects.filter(code=code)
    if len(qs)==0:
        ...
        if request.method=='POST':
            ...
            if form.is_valid():
                ...
                m=Meeting.objects.filter(code=code).first()
    ...
    return render(request,'add_minutes.html',{'form':form,'msg':msg})
...
def meeting_stats(request):
    ...
    return render(request, "meeting_stats.html", {"meeting_codes": meeting_codes})


def member_stats(request):
    member_names = Member.objects.values_list("name", flat=True).distinct()

    if request.method == "POST":
        member_name = request.POST.get("member_name")
        if not member_name:
            return render(request, "member_stats.html", {"member_names": member_names, "error": "No member selected."})
    ...
    return render(request, "member_stats.html", {"member_names": member_names})
